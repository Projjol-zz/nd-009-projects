...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        # print '*'*10
        # print "current q state {}".format(self.Q)
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            if self.epsilon > random.random():
                ...
                print "out here"
                # get action with the highest Q-value for the current state
                max_q_value = self.get_maxQ(state)
                max_q_list = [] #in case more than one state has the same max value
                for key, value in self.Q[state].iteritems():
                    if value == max_q_value:
                        # action = key
                        max_q_list.append(key)
                return random.choice(max_q_list)                
        else:
            action = random.choice(self.valid_actions)
        return action
